...
var expired = flag.Int("expired", 60, "session expired in seconds")
...
func handleSession(f *Forwarder, key string, session Session) {
    ...
    fmt.Printf("(start) %s <%s>", key, session.serverConn.LocalAddr())
    ...
    fmt.Printf("(end) %s <%s>", key, session.serverConn.LocalAddr())
}
    ...
    for {
        ...
        if session, found := f.sessions[key]; found {
            ...
            if time.Now().Unix() > expiredTime && *expired > 0 {
                ...
                lastConn = session.serverConn
                fmt.Printf("(delete) <%s>\n", f.sessions[key].serverConn.LocalAddr())
                delete(f.sessions, key)
            }
        } else if serverConn, err := net.DialUDP("udp", nil, f.toAddr); err == nil {
            fmt.Printf("(new) Write to %s key %s <%s>\n", f.toAddr.String(), key, serverConn.LocalAddr())
            ...
            expiredTime = time.Now().Add(time.Second * time.Duration(*expired)).Unix()
            serverConn.SetDeadline(time.Now().Add(time.Second * time.Duration(*timeout + *expired)))
